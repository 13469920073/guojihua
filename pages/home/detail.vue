  ...
  <view>
    <view v-if="value.pid">
      <view style="padding: 10px;">
        <view v-if="postCategory != 'sy'"
          style="font-size: 20px; color: #111;padding: 2px 10px ; padding-bottom: 10px;font-weight: 400;">
          <text v-if="_postSubType(value)"
            style="padding: 0px 10px; color: #fff; background-color: #FF8000;border-radius: 5px;font-size: 16px;margin-right: 5px;">{{
      _postSubType(value) }}</text>{{ postTitle(value) }}
        </view>
        <view style="display: flex;align-items: center;padding-top: 6px;">
          ...
          <view style="margin-left: 12px;color: #666;">
            <view style="line-height: 1.5;">
              <text style="font-size: 15px;color: #444;">{{ value.content.user.name ||
      value.content.user.nickName }}</text>
            </view>
            <view style="line-height: 1.5;">
        ...
        <view v-if="imgArr.length > 0" style="padding:5px;">
          <view v-for="(v, i) in imgArr" :key='i' style="display: flex;flex-direction: row;">
            <view v-for="(ig, k) in v" :key="k" style="display: flex;flex-direction: column;">
              <image v-if="!igLenMore_3" v-bind:src="ig" mode="widthFix"
                v-bind:style="{ width: igW + 'px', borderRadius: '2px', padding: '2px', flexGrow: 2 }"></image>
              <image v-else v-bind:src="ig" mode="aspectFill"
                v-bind:style="{ width: igW + 'px', height: igW * 0.8 + 'px', borderRadius: '2px', padding: '2px', flexGrow: 2 }">
              </image>
            </view>
          </view>
        </view>
        <view style="margin-top: 15px;margin-left: 5px;">
          <view style="display: flex;align-items: center;">
            ...
            <view style="margin-right: 40px;font-size: 12px;color: #808080;display: flex;">
              <image src="../../static/images/comment_like_icon_normal@2x.png"
          ...
          <view style="margin-top: 10px;font-size: 12px;color: #666;">
            提示: 平台不对该信息承担任何责任，若发现虚假信息等违法行为请 <text v-on:click="connectUsAction(0)"
              style="color: #FF5053;padding: 3px 0;margin: 0 5px;font-size: 13px;">点击举报 </text> 一经核实系统立即删除
          </view>
        </view>
      </view>
      <!-- 评论 -->
      <view style="padding: 0;">
        <view
          style="margin-top: 8px;  border-width: 0; border-width: 8px 0 1px 0;border-color: #F8F8F8; border-bottom-color: #eee; border-style: solid;padding: 10px 10px;font-size: 15px;color: #888;">
          评论</view>
      ...
      <view class="bottom-bar" style="display: flex; align-items: center;">
        <view style="display: flex; align-items: center;width: 100%;">
          <view
            style="border-width: 0;background-color: #f5f5f5;display: flex;margin-left: 10px; padding: 6px 30px;align-items: center;border-radius: 15px;width: 50%;justify-content: center;">
          ...
          <view style="display: flex; align-items: center; justify-content: space-around;width: 100%;">
            <view style="display: flex;">
              ...
              <text v-if="0 > 0"
                style="background-color: #FF5053;color: white;font-size: 10px;margin-left: -12px;padding: 0px 5px; border-radius: 6px;height: 15px;"></text>
            </view>
            <view style="font-size: 12px;color: #808080;display: flex;align-items: center;">
              ...
              <image v-else src="../../static/images/tab_collect@2x.png" style="width: 25px; height: 25px;"></image>
            </view>
            <image src="../../static/images/share_icon@2x.png" style="width: 25px; height: 25px;"></image>
          </view>
        </view>
      </view>
    </view>
